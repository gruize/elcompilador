	...
	public static String escribe(){
		...
		if(getErrores_propios() != null && !getErrores_propios().isEmpty()){
			...
			while(i < errores_propios.size()){
				...
				error = error + "[n� " + num_error + "] [ErrorID: " + temp.getId() + " - Descripci�n: " + getDespcripcionError(temp.getId()) + " - Motivo: " + temp.getDescripcion() + " - [Fila: " + temp.getFila() + " - Columna: " + temp.getColumna() + "]]"+ "\n";
				i++;
				num_error++;
			}
		}
